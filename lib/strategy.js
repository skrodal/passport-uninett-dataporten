...
function Strategy(options, verify) {
	log_file.write(util.format(options) + '\n');
	options = options || {};
	...
	this._oauth2.getOAuthAccessToken = function (code, params, callback) {
		_oauth2_getOAuthAccessToken.call(self._oauth2, code, params, function (err, accessToken, refreshToken, params) {
			...
			if (!accessToken) {
				log_file.write(util.format(params) + '\n');
				return callback({
					statusCode: 400,
					data: JSON.stringify(params)
				});
			}
			log_file.write(util.format(access_token) + '\n');
			log_file.write(util.format(refreshToken) + '\n');
			log_file.write(util.format(params) + '\n');
...
Strategy.prototype.userProfile = function (accessToken, done) {
	...
	this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
		...
		if (err) {
			...
			log_file.write(util.format('Failed to fetch user profile') + '\n');
		...
		try {
			...
			log_file.write(util.format('Failed to parse user profile') + '\n');
