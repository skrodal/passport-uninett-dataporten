...
var $require = require('proxyquire')
  ...
  , DataportenStrategy = require('../lib/strategy');
...
describe('Strategy', function() {
  ...
  describe('constructed', function() {
    ...
    it('should be named dataporten', function() {
      expect(strategy.name).to.equal('dataporten');
    });

    it('should have default user agent', function() {
      expect(strategy._oauth2._customHeaders['User-Agent']).to.equal('passport-uninett-dataporten');
    });
  })
  ...
  describe('constructed with undefined options', function() {
    it('should throw', function() {
      expect(function() {
        var strategy = new DataportenStrategy(undefined, function(){});
      }).to.throw(Error);
    });
  })
  ...
  describe('handling a response with an authorization code', function() {
    ...
    it('should authenticate user', function() {
      ...
      expect(user.username).to.equal('borson@olderdalen.no');
    });
  });
    ...
    var MockOAuth2Strategy = function(options, verify) {
      ...
      this._oauth2.getOAuthAccessToken = function(code, options, callback) {
        return callback(null, undefined, undefined, {
          ...
          error_uri: 'https://developer.dataporten.com/v3/oauth/#bad-verification-code' });
      };
    }
    util.inherits(MockOAuth2Strategy, OAuth2Strategy);
  ...
  describe('error caused by invalid code sent to token endpoint, with response correctly indicating success', function() {
    ...
    var MockOAuth2Strategy = function(options, verify) {
      ...
      this._oauth2.getOAuthAccessToken = function(code, options, callback) {
        return callback({
          statusCode: 401,
          data: '{"error":"bad_verification_code","error_description":"The code passed is incorrect or expired.","error_uri":"https://developer.dataporten.com/v3/oauth/#bad-verification-code"}' });
      };
    }
    util.inherits(MockOAuth2Strategy, OAuth2Strategy);
